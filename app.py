...
def hybrid_recommend(customer_id, matrix, sparse, model, sim_df, idx_to_cust, cust_to_idx, products, top_n=10):
    ...
    combined = 0.6 * neighbor_scores + 0.4 * item_scores
    ...
    # Step 1: Get top-N based on hybrid rec_score
    top_by_hybrid = rec_with_sentiment.sort_values('rec_score', ascending=False).head(top_n)
    
    # Step 2: Sort those by sentiment_score
    top_recommendations = top_by_hybrid.sort_values('sentiment_score', ascending=False)
    
    # Merge with full product details
    final_recommendations = pd.merge(
        top_recommendations[['product_id']],
        products,
        on='product_id',
        how='left'
    )
...
def main_page():
    ...
    if search_query:
        filtered = products[
            ...
            products['sub_category'].str.contains(search_query, case=False, na=False)
        ]
